...
interface SaleReturnItem extends BaseProductItem {
    quantity: number
    maxQuantity?: number
    netUnitPrice: number
    discount: number
...
interface SaleReturnFormProps {
    onSubmit: (data: CreateSaleReturnPayload) => void
    onCancel: () => void
    isLoading?: boolean
...
const SaleReturnForm = ({
    ...
    // =========================
    // FETCH SALE DETAIL IF SALE ID PROVIDED
    // =========================
    const { data: saleDetail, isLoading: isLoadingSale } = useGetSaleDetail(
        initialSaleId ? Number(initialSaleId) : undefined
    )

    const { items: returnItems, setItems, updateItem, deleteItem } = useProductSelection<SaleReturnItem>({
        products: [],
        ...
        onItemUpdate: (item, field) => {
            if (field === 'quantity') {
                const maxQty = (item as SaleReturnItem).maxQuantity
                const qty = maxQty 
                    ? Math.min(Math.max(1, item.quantity || 1), maxQty)
                    : Math.max(1, item.quantity || 1)
                const subtotal = item.netUnitPrice * qty
                return { quantity: qty, subtotal } as Partial<SaleReturnItem>
            }
            return {}
        }
    })
    ...
    // =========================
    // POPULATE ITEMS FROM SALE DETAIL
    // =========================
    useEffect(() => {
        if (saleDetail && saleDetail.items) {
            const items: SaleReturnItem[] = saleDetail.items.map((item, index) => {
    ...
    const handleSubmit = () => {
        ...
        if (!initialSaleId || !saleDetail || !reason.trim()) {
            return
        }

        const payload: CreateSaleReturnPayload = {
            sale_id: Number(initialSaleId),
            reason: reason.trim(),
            store_id: saleDetail.store.id,
    ...
    const isSubmitDisabled = selectedItems.length === 0 || !initialSaleId || !saleDetail || !reason.trim() || isLoadingSale
    ...
    if (!initialSaleId || !saleDetail) {
        return (
            <DashboardCard bodyClassName='space-y-2'>
                <div className="text-center py-8">
                    <p className="text-gray-500">Please select a sale to create a return.</p>
                </div>
            </DashboardCard>
        )
    }
