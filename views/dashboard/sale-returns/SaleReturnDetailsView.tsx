...
import { useGetSaleReturnDetail } from '@/services'
import moment from 'moment'
import { notFound } from 'next/navigation'

interface SaleReturnDetailsViewProps {
    returnId: string
}

const SaleReturnDetailsView = ({ returnId }: SaleReturnDetailsViewProps) => {

    // ================================
    // FETCH SALES RETURN DETAILS
    // ================================
    const { data: returnDetail, isLoading, error } = useGetSaleReturnDetail(Number(returnId))
    ...
    return (
        <>
            ...
            <div className="p-3 space-y-3">
                {/* ======================== SALE & STORE INFORMATION ======================== */}
                <DashboardCard 
                    title="Sale & Store Information"
                    className='p-1' 
                    bodyClassName='grid grid-cols-1 md:grid-cols-2 gap-5'
                >
                    <div className="space-y-2">
                        <h6 className="text-xs text-gray-600">Sale Reference</h6>
                        <p className="text-sm font-medium text-gray-900">{returnDetail.sale_reference}</p>
                    </div>
                    <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                        ...
                        <p className="text-sm font-medium text-gray-900">{returnDetail.customer_name}</p>
                    </div>
                    <div className="space-y-2">
                ...
                >
                    <div className="space-y-2">
                        ...
                        <p className="text-sm font-medium text-gray-900">#{returnDetail.sale_return_id}</p>
                    </div>
                    <div className="space-y-2">
                        <h6 className="text-xs text-gray-600">Total Refund</h6>
                        <p className="text-sm font-medium text-gray-900">{formatCurrency(returnDetail.total_refund)}</p>
                    </div>
                ...
                <DashboardCard title="Return Items" className='p-1'>
                    <TableComponent
                        ...
                        rowKey={(item) => String(item.sale_return_item_id || `${item.product_id}-${item.variation_id || 'simple'}`)}
                        renderRow={(item) => {
...
export default SaleReturnDetailsView
